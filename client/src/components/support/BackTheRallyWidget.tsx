...
export function BackTheRallyWidget({
  ...
  return (
    <button
      ...
      className={`
        group relative inline-flex flex-col items-center shrink-0
        ...
        ${fullWidth ? "w-full py-3" : "py-1"}
        ${className}
      `}
    >
      {/* Top line: the "brush stroke"/highlighter mark behind the text
          - a real SVG shape (an earlier version used a Tailwind
          arbitrary-value border-radius with a "/" in it, which
          Tailwind almost certainly parsed as an opacity-modifier
          separator instead of part of the radius value, silently
          dropping the whole utility). preserveAspectRatio="none"
          stretches this exact hand-drawn-ish path to fill the button
          regardless of how wide "Back the Rally" actually renders, so
          this doesn't need to know the text's pixel width ahead of
          time. Wrapped in its own relative span so the highlight only
          covers this line, not the subtitle below it. */}
      <span className="relative inline-flex items-center gap-1.5">
        <svg
          ...
          className="
            absolute -left-1.5 -right-1.5 -top-0.5 bottom-[3px]
      ...
      {/* Subtitle line, plain text below the highlighted line - no
          highlighter mark of its own, just a smaller/lighter caption
          matching the reference. */}
      <span className="text-[11px] italic text-muted-foreground mt-0.5 whitespace-nowrap">
