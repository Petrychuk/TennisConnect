...
export interface TravelFormProps {
  mode?: "create" | "edit";
  initialData?: any; // full travel package record when editing
  onClose: () => void;
  onSaved?: () => void;
}

const ALL_FIELDS = FIELDS.travel as AdminFieldDef[];

const DETAIL_FIELDS = ALL_FIELDS.filter(
  (f) => f.name !== "coverImage" && f.name !== "gallery"
);

const SWITCH_FIELDS: Record<string, string> = {
  isFeatured: "Show this package as Featured on TennisConnect.",
};

function blankForm(): Record<string, any> {
  const data: Record<string, any> = {};
  ALL_FIELDS.forEach((f) => {
    data[f.name] = f.name === "gallery" ? [] : f.type === "checkbox" ? false : "";
  });
  return data;
}
...
function buildPayload(form: Record<string, any>) {
  ...
  ALL_FIELDS.forEach((f) => {
    ...
    if (f.name === "gallery") {
      payload.gallery = Array.isArray(v) ? v : [];
      return;
    }
...
export function TravelForm({
  ...
  const handleSave = async () => {
    try {
      ...
      // New packages start hidden — the admin explicitly publishes once
      // they've reviewed the media step / full preview.
      if (mode === "create") payload.isActive = false;

      const url =
        mode === "create"
          ? "/api/admin/travel"
          : `/api/admin/travel/${savedId}`;
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || "Failed to save travel package");
      }
      ...
      setForm((prev) => ({
        ...
        gallery: Array.isArray(saved.gallery) ? saved.gallery : prev.gallery,
      }));
      setStep("media");
    } catch (err: any) {
      toast({
  ...
  const saveMedia = async (
    data: Partial<{ coverImage: string; gallery: string[] }>
  ) => {
    if (!savedId) return;
    try {
      const payload = { ...buildPayload(form), ...data };

      const res = await fetch(`/api/admin/travel/${savedId}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
  ...
  const handleDone = () => {
    toast({
      description:
        mode === "create"
          ? "Travel package has been successfully created."
          : "Travel package has been successfully updated.",
    });
    onSaved?.();
    onClose();
  };
  ...
  return (
    <div className="space-y-10" data-testid="travel-form">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">
            {step === "details" ? "Package Details" : "Upload Media"}
          </h2>
      ...
      {/* STEP 2 */}
      {step === "media" && (
        <div className="space-y-8" data-testid="travel-media-section">
          <div>
            <h3 className="text-lg font-semibold">Package Images</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              Upload a cover image and a gallery of photos for this package.
            </p>
          </div>
          ...
          <ImageUploader
            ...
            label="Cover Image"
            description="Large hero image displayed at the top of the package page."
            allowUrl
          ...
          <GalleryUploader
            ...
            max={10}
            label="Gallery"
            description="Upload up to 10 photos showcasing this package."
            onChange={async (urls) => {
        ...
        {step === "details" ? (
          ...
          <>
            <Button
              variant="outline"
              onClick={handleBack}
              data-testid="travel-back-btn"
            >
              ← Back
            </Button>

            <Button onClick={handleDone} data-testid="travel-done-btn">
              Done
            </Button>
          </>
        )}
